...
func App() *buffalo.App {
	if app == nil {
		...
		app.Use(Authorize)
		app.Middleware.Skip(Authorize, HomeHandler, LoginHandler)
		bah := buffalo.WrapHandlerFunc(gothic.BeginAuthHandler)
		auth := app.Group("/auth")
		auth.GET("/{provider}", bah)
		auth.DELETE("", AuthDestroy)
		auth.Middleware.Skip(Authorize, bah, UserAuthCallback)
		auth.GET("/{provider}/callback", UserAuthCallback)
		scm := auth.Group("/scm")
		scm.Use(SetCurrentUser)
		scm.Use(Authorize)
		scm.GET("/{provider}", bah)
		scm.GET("/{provider}/callback", ScmAuthCallback)
